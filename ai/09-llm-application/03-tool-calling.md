...
核（kernel /ˈkɜːrnl/）心价值：
- **连接实时数据**：查询数据库、调用 API、获取天气/股价
- **执行操作**：发送邮件、创建订单、控制设备
- **增强推理（inference /ˈɪnfərəns/）**：让 LLM 借助计算器、代码解释器等工具解决纯文本难以处理的问题
...
要让 LLM 正确调用工具，需要提供**工具定义**——包括名称、描述和参数（parameter /pəˈræmɪtər/） Schema。**描述信息至关重要**，LLM 据此判断何时使用哪个工具。
