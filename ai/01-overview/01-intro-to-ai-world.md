...
常见的机器学习方法包括 **支持向量机（Support Vector Machine, SVM）**、**随机森林（Random Forest, RF）**、**梯度提升树（Gradient Boosting Decision Tree, GBDT）** 等。

### 什么是 Deep Learning（深度学习）？

**Deep Learning** 是机器学习的一个子集。它的核心是"神经网络（Neural Network）"——一种模仿大脑神经元连接方式的计算结构。当神经网络的层数很多时（比如几十层甚至上百层），我们就叫它"深度"学习。

深度学习在三个领域掀翻了之前的所有方法：

- **计算机视觉（Computer Vision）**：用 **卷积神经网络（Convolutional Neural Network, CNN）** 识别图片
- **序列建模（Sequence Modeling）**：用 **RNN（Recurrent Neural Network）/ LSTM（Long Short-Term Memory）** 处理语音、文本等序列数据
- **现代基石——Transformer**：2017 年 Google 提出 **Transformer** 架构，它用 **自注意力机制（Self-Attention Mechanism）** 彻底取代了 RNN，成为今天几乎所有大模型的基础

### 什么是 Generative AI（生成式 AI）？

**GenAI** 是深度学习发展到一定阶段的产物。以前的 AI 更多是"识别"和"分类"——这张图是猫还是狗、这段文字是正面还是负面情绪。而生成式 AI 能创造新内容：

- **文本生成**：ChatGPT、Claude（基于 **大语言模型（Large Language Model, LLM）** + **基于人类反馈的强化学习（Reinforcement Learning from Human Feedback, RLHF）**）
- **图像生成**：Midjourney、Stable Diffusion（基于 **扩散模型（Diffusion Model）**）
...
**AI Agent** 是 2023 年以后最热门的方向。一个 Agent 不只是"生成文字"，而是一个能**自主行动**的系统：
...
| **ML** | AI 的子集 | 垃圾邮件过滤、推荐系统 | SVM、随机森林、线性回归 | 有大量数据、规则难以手工编写时 |
