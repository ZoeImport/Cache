...
> **Python 的机器学习生态已经成熟。** 从 HuggingFace 的统一模型接口到 vLLM 的高效推理（inference /ˈɪnfərəns/），从 PyTorch Lightning 的训练框架到 LangChain 的 LLM 应用编排，工具链正在快速收敛。这一章不深入任何框架的内部，而是带你俯瞰整个生态的版图。
>
...
核（kernel /ˈkɜːrnl/）心库 [Transformers](https://github.com/huggingface/transformers) 提供了统一的 `from_pretrained()` 接口，无论模型架构如何（BERT、GPT、LLaMA、Whisper、ViT），加载和推理的方式几乎一致：
...
- **Task-specific Heads**: 相同的 backbone 可以接不同的输出头（分类（classification /ˌklæsɪfɪˈkeɪʃən/）、生成、问答、摘要）
...
[PEFT](https://github.com/huggingface/peft) (Parameter（/pəˈræmɪtər/）-Efficient Fine-Tuning) 让你在不改变全部参数的情况下微调大模型：

- **LoRA**: 低秩适配矩阵，仅训练新增的小矩阵
- **QLoRA**: 在 LoRA 基础上将基座模型 4-bit 量化（quantize /ˈkwɒntaɪz/），单卡可以微调 65B 模型
...
- **超参搜索 (Sweeps)**: 自动化的贝叶斯 / 网格 / 随机（stochastic /stəˈkæstɪk/）搜索
...
- **Agent（/ˈeɪdʒənt/） + Tools**: 结合 LangChain 风格的 Agent
...
- **极致优化**: K/V Cache 量化、Flash Attention（/əˈtenʃən/）、NUMA 感知
