...
> **大语言模型（LLM）的"通识教育"使其具备广泛能力，但要让模型在特定任务（如法律问答、医疗诊断、代码生成）上表现出色，**微调 (Fine-Tuning)** 是关键步骤。本章从全量微调的高昂成本出发，引出参数高效微调 (PEFT) 方法，深入讲解 LoRA 的数学原理、QLoRA 的量化技巧，以及 Adapter / Prefix Tuning 等经典方案，并辅以完整代码实现。**
>
...
**前置知识 (Prerequisites):** Transformer 架构 (Chapter 1, 2), PyTorch 基础, 基本的线性代数 (矩阵乘法、秩)
...
全量微调需要：
1. **存储完整优化器状态** (Adam 需 2× 额外内存：动量和方差)
2. **存储梯度** (又 1× 参数量)
3. **存储模型参数** (至少 1×)
4. **反向传播通过全部层** (计算量巨大)
...
**PEFT (Parameter-Efficient Fine-Tuning)** 的核心思想：**冻结预训练模型的大部分参数，只更新少量新增或选中的参数**。
...
| Quality on low-resource tasks | Overfitting risk | More robust |
...
其中 $\alpha$ 是缩放超参数（通常 $\alpha = 2r$ 或 $\alpha = 16$）。

推理时使用 $W'$ 替代 $W_0$，**完全不需要额外计算或内存开销**：
...
|:---------|:------|:---------------:|:-------:|
| Prompt Tuning | Only input embedding layer | $k \times d_{\text{model}}$ | Lower |
