...
> **"填空"是最优雅的自监督学习方式。BERT 通过 Masked Language Model（MLM）在 NLP 领域掀起革命，MAE（Masked Autoencoder）则将同样的思想推广到计算机视觉。两个方法的核心理念高度一致：遮挡输入的一部分，让模型学会预测被遮挡的内容——这个简单的过程迫使模型学习到深层的语义理解。**
>
...
**前置知识 (Prerequisites):** Transformer Encoder (第5章第2节)、自监督学习基本概念 (第6章第1节)
...
BERT 的输入由三个嵌入相加构成，每个 token 最终表示为：
...
- **[CLS]**: 分类标记，其最后一层输出用作整个序列的表示（用于 NSP 和下游分类任务）
- **[SEP]**: 分隔标记，分隔两个句子
...
MLM 的核心操作是随机遮挡 15% 的 token，然后让模型预测它们。但 BERT 使用了一种**精心设计的掩码策略**：
...
BERT 使用 **Transformer Encoder**——每个 block 包含 Multi-Head Self-Attention + Feed-Forward Network，每个子层后接残差连接和 Layer Normalization。
...
3. **解码：** 预测头（线性层 + softmax）恢复原始 token
...
3. **解码：** 用轻量级 Decoder 从编码特征和可学习的 mask tokens 中重建完整图像
...
- 这远高于 BERT 的 15% 掩码率
- 图像有高度的空间冗余——相邻像素高度相关
- 75% 的掩码几乎消除了所有"简单"线索（如颜色扩散、纹理延拓），迫使模型理解**高阶语义**
...
- **T5 (Text-to-Text Transfer Transformer):** 将一切任务都转化为 text-to-text 格式，使用 Encoder-Decoder 架构，Encoder 双向、Decoder 自回归
