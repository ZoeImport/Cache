...
> 在经典机器学习（Classical Machine Learning）中，我们最关心的问题是：**训练好的模型在未见过的数据上表现如何？** 本章围绕模型评估（Model Evaluation）与验证（Validation）展开，涵盖过拟合与欠拟合的诊断（Overfitting vs Underfitting）、数据集的正确划分（Train/Val/Test Split）、交叉验证（Cross-Validation）、学习曲线（Learning Curves）以及分类任务的核心评估指标（Metrics）。**学习曲线是本章的重点**——它是诊断模型状态的"X光片"。
...
| 数据集 | 用途 | ⚠️ 禁止行为 |
|:---|:---|:---|
| **训练集（Training Set）** | 训练模型参数 | 不能用于调整超参数 |
...
| 泄露类型 | 示例 | 正确做法 |
|:---|:---|:---|
| **特征泄露** | 用全体数据的均值和标准差做归一化 | **先划分再归一化**，只计算训练集的统计量 |
...
- **对策**：增加模型复杂度（更多特征、更高阶多项式、更深网络）、减少正则化
...
- **对角线（Random Classifier）**：完全随机猜测，AUC = 0.5
- 左上角点（0,1）：完美分类器

#### AUC（Area Under the ROC Curve）

**AUC** 是 ROC 曲线下的面积，是一个标量指标：
...
*下一章：[03 — 线性模型与正则化](./03-linear-models.md)*
